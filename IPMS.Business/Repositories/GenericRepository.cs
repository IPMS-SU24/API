...
namespace IPMS.Business.Repository
{
    public class GenericRepository<TEntity> : IGenericRepository<TEntity> where TEntity : BaseModel
    {
        ...
        {
            return dbSet.ApplySoftDeleteFilter(_context).AsNoTracking();
        }

        public virtual async Task<TEntity?> GetByIDAsync(object id)
        {
            var technicalId = Guid.Parse(id.ToString());
            var result = await dbSet.ApplySoftDeleteFilter(_context).Where(x=>x.Id == technicalId).FirstOrDefaultAsync();
            return result;
        }
